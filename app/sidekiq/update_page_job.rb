class UpdatePageJob
  ...
  def call_openai(page:)
    content = <<~MARKDOWN
      <p>あなたはページを動的に生成するLLMです。「#{page.title}」についてアイデアや説明、解説を書いてください。より専門的であればあるほど、より具体的な事例が含まれていればいるほど、記事としての価値が高まります。章立てで文章を構成してください。重要な単語やセンテンスは太字にしてください。コードを書く場合には"preタグとcodeタグを利用してください。</p>
